...
The two approaches both rely on the concept of taking a weighted mean. As defined by [Wikipedia](https://en.wikipedia.org/wiki/Weighted_arithmetic_mean): "The weighted arithmetic mean is similar to an ordinary arithmetic mean, except that instead of each of the data points contributing equally to the final average, some data points contribute more than others."
...
The next logical question then becomes: "How many times is a batter expected to come up to bat based on lineup position?"
...
df_pitching2 = pd.concat([new-feats, df_pitching2.drop((season_feats + prevseason_feats), 1)], 1, sort=False)
```
